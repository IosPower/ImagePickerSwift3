...
open class ImageScrollView: UIScrollView, UIScrollViewDelegate {
    ...
    func adjustFrameToCenter() {
        ...
        if frameToCenter.size.height < bounds.height {
            frameToCenter.origin.y = ((bounds.height - frameToCenter.size.height) / 2)-32
            
            // here 32 because  navigation height 64 so imageview middle purpose
        }
        else {
    ...
    open func displayImage(_ imageURL: URL) {
        ...
        downloadedFrom(url: imageURL)
        self.bringSubview(toFront: self.indicator)
    }
    
    open func displayImageDefault(_ image: UIImage) {
        if let zoomView = zoomView {
            zoomView.removeFromSuperview()
        }
        showFromImage(image: image)
    }

    func showFromImage(image: UIImage) {
        let imageView: UIImageView = UIImageView.init()
        imageView.image = image
        
        self.zoomView = UIImageView(image: imageView.image)
        ...
        if imageView.image != nil {
            self.configureImageForSize((imageView.image!.size))
        }
    }
